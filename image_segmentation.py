...
from seg import mask_rcnn, seg_data_visualizer
...
parser.add_argument('-v', '--view_list', nargs='+', default=['0', '5', '7'], help='List of cameras visualised : [0, 1, ... , 6]')
		...
		for j, f in enumerate(files_path):
			...
			if cam_id in args.camera_list and str(img_format) in args.view_list:
				...
				if img_format==0:
					...
					pred = mrcnn.predict(dataset).iloc[0]
					boxes = pred['boxes']
					scores = pred['scores']
					...
					vis_output_img = viz.overlay_instances(
						img,
						masks=masks,
						boxes=boxes,
						labels=classes,
					...
					#grid_display.imshow(grid_index+(plots_width-1), depth)
					grid_display.imshow(special_plot[cam_id], vis_output_img)
