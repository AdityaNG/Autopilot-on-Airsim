...
parser.add_argument('-v', '--view_list', nargs='+', default=['0', ], help='List of cameras visualised : [0, 1, ... , 6]')
		...
		for j, f in enumerate(files_path):
			...
			if cam_id in args.camera_list and str(img_format) in args.view_list:
				...
				if img_format==0:
					...
					img_res = img.copy()
					for index, lab in detections.iterrows():
						img_res = cv2.rectangle(img_res, (round(lab['xmin']), round(lab['ymin'])), (round(lab['xmax']), round(lab['ymax'])), (255,255,0),2)
					# grid_display.imshow(grid_index+plots_width-1, depth)
					#cv2.imshow('depth', depth)
					#grid_display.imshow(grid_index*(plots_width), depth)
					#grid_display.imshow(grid_index+(plots_width-1), depth)
					grid_display.imshow(special_plot[cam_id], img_res)
